...
import com.vertical.app.module.work.routine.RoutineManagementAdapter.TextHeaderViewHolder;
import com.vertical.app.module.work.routine.RoutineManagementAdapter.TransactionManagementItemViewHolder;
...
public class RoutineManagementAdapter extends SectionedRecyclerViewAdapter<TextHeaderViewHolder,TransactionManagementItemViewHolder,RecyclerView.ViewHolder> {
    private Context mContext;
    private LayoutInflater mInflater;
    private SparseArray<List<RoutineManagementMenu>> mData;
    private Callback mCallback;

    public RoutineManagementAdapter(Context context, SparseArray<List<RoutineManagementMenu>> data){
        mContext = context;
    ...
    protected TransactionManagementItemViewHolder onCreateItemViewHolder(ViewGroup parent, int viewType) {
        return new TransactionManagementItemViewHolder(mInflater.inflate(R.layout.item_routine_management,parent,false));
    }

    @Override
    protected void onBindSectionHeaderViewHolder(TextHeaderViewHolder holder, int section) {
        if (section == 0){
            holder.header.setText("事务跟进");
        }else {
//            holder.header.setText("订单管理");
    ...
    protected void onBindItemViewHolder(TransactionManagementItemViewHolder holder, int section, int position) {
        final RoutineManagementMenu item = mData.get(section).get(position);
    ...
    public interface Callback {
        void onItemClick(RoutineManagementMenu item);
    }
