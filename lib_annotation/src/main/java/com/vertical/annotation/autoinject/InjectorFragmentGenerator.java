...
public class InjectorFragmentGenerator {
    ...
    public InjectorFragmentGenerator(Filer filer, Messager messager, Types typeUtils) {
        mFiler = filer;
        mMessager = messager;
        mTypeUtils = typeUtils;
        mTypeSpecBuilder = TypeSpec.classBuilder(InjectorFragmentName);
    }

    private void generateModifier() {
        mTypeSpecBuilder.addModifiers(Modifier.PUBLIC, Modifier.ABSTRACT)
                .superclass(ClassName.get(CorePackageName, "BaseRxFragment"));
    }
    ...
    private void generateCodeBlock(Element element) {
        ...
        String code = String.format("%s.builder().%s(new $L(getActivity().getApplication(), this)).build().inject(($L)this)", "Dagger" + ComponentName, moduleMethodName);
